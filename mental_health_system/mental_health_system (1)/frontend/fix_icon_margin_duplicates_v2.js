...
function fixIconMarginDuplicates() {
    ...
    function processHTMLFile(filePath) {
        try {
            ...
            // 修复重复的 mr-1 类
            const mr1Regex = /class="([^"]*?)mr-1\s+mr-1([^"]*?)"/g;
            if (mr1Regex.test(content)) {
                content = content.replace(mr1Regex, 'class="$1mr-1$2"');
                hasChanges = true;
            }

            // 修复重复的 mr-2 类
            const mr2Regex = /class="([^"]*?)mr-2\s+mr-2([^"]*?)"/g;
            if (mr2Regex.test(content)) {
                content = content.replace(mr2Regex, 'class="$1mr-2$2"');
                hasChanges = true;
            }
            ...
            if (hasChanges) {
                ...
                fixedFiles++;
                console.log(`已修复图标margin重复: ${filePath}`);
            }
