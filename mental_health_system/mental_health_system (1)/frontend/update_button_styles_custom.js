...
// 自定义修改按钮样式：修改注册按钮字体颜色（不用白色），确保登录按钮外框为蓝色
function updateButtonStylesCustom() {
    const frontendDir = '.';
    let totalFiles = 0;
    ...
    function processHTMLFile(filePath) {
        try {
            ...
            // 2. 修改桌面端注册按钮样式 - 修改字体颜色（不用白色，使用浅灰色）
            ...
            if (desktopRegisterButtonRegex.test(content)) {
                content = content.replace(desktopRegisterButtonRegex, match => {
                    ...
                    // 创建新的注册按钮，使用浅灰色文本（不用白色）
                    const newRegisterButton = `<button class="px-4 py-2 bg-gradient-to-r from-primary to-indigo-600 text-gray-100 rounded-lg hover:from-indigo-600 hover:to-indigo-700 transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-primary/30 font-bold shadow-md hover:shadow-lg transform hover:-translate-y-0.5 active:translate-y-0" onclick="window.location.href='register.html'">
            ...
            if (mobileButtonLoginRegex.test(content)) {
                content = content.replace(mobileButtonLoginRegex, match => {
                    ...
                    // 为移动端按钮设置正确的样式
                    const newMobileLoginButton = `<button class="px-3 py-2 text-primary border-2 border-primary rounded-lg hover:bg-primary/5 hover:text-primary transition-all duration-300 text-sm w-full" onclick="window.location.href='login.html'">
                        <i class="${iconClass} mr-1"></i>登录
                    </button>`;
            ...
            if (mobileButtonRegisterRegex.test(content)) {
                content = content.replace(mobileButtonRegisterRegex, match => {
                    ...
                    // 为移动端按钮设置正确的样式
                    const newMobileRegisterButton = `<button class="px-3 py-2 bg-gradient-to-r from-primary to-indigo-600 text-gray-100 rounded-lg hover:from-indigo-600 hover:to-indigo-700 transition-all duration-300 text-sm w-full shadow-md hover:shadow-lg" onclick="window.location.href='register.html'">
            ...
            if (mobileLinkLoginRegex.test(content)) {
                content = content.replace(mobileLinkLoginRegex, match => {
                    ...
                    // 为移动端链接按钮设置正确的样式
                    const newMobileLoginLink = `<a href="login.html" class="block px-4 py-3 text-primary border-2 border-primary rounded-lg hover:bg-primary/5 hover:text-primary transition-all duration-300 text-sm w-full">
            ...
            if (mobileLinkRegisterRegex.test(content)) {
                content = content.replace(mobileLinkRegisterRegex, match => {
                    ...
                    // 为移动端链接按钮设置正确的样式
                    const newMobileRegisterLink = `<a href="register.html" class="block px-4 py-3 bg-gradient-to-r from-primary to-indigo-600 text-gray-100 rounded-lg font-bold flex items-center transition-all duration-300 text-sm w-full shadow-md hover:shadow-lg">
                        <i class="${iconClass} mr-2 w-5 text-center"></i>
                        注册
                    </a>`;
