...
// 更新按钮样式：将注册按钮文字颜色改为更深且协调的颜色（不用白色），确保登录按钮外框为蓝色
function updateButtonStylesDarkText() {
    const frontendDir = '.';
    let totalFiles = 0;
    ...
    function processHTMLFile(filePath) {
        try {
            ...
            // 2. 修改桌面端注册按钮样式 - 修改字体颜色为更深且协调的颜色（使用深蓝色）
            ...
            if (mobileButtonRegisterRegex.test(content)) {
                content = content.replace(mobileButtonRegisterRegex, match => {
                    ...
                    // 为移动端按钮设置正确的样式
                    const newMobileRegisterButton = `<button class="px-3 py-2 bg-gradient-to-r from-primary to-indigo-600 text-indigo-900 rounded-lg hover:from-indigo-600 hover:to-indigo-700 transition-all duration-300 text-sm w-full shadow-md hover:shadow-lg" onclick="window.location.href='register.html'">
            ...
            if (mobileLinkRegisterRegex.test(content)) {
                content = content.replace(mobileLinkRegisterRegex, match => {
                    ...
                    // 为移动端链接按钮设置正确的样式
                    const newMobileRegisterLink = `<a href="register.html" class="block px-4 py-3 bg-gradient-to-r from-primary to-indigo-600 text-indigo-900 rounded-lg font-bold flex items-center transition-all duration-300 text-sm w-full shadow-md hover:shadow-lg">
                        <i class="${iconClass} mr-2 w-5 text-center"></i>
                        注册
                    </a>`;
